...
class Tracker extends Parse {
	...
	async remove(data) {
		try {
			...
			if (!found) throw "Couldn't find username.";
			for (let account in data.dbuser[data.source.key]) {
				if (!data.dbuser[data.source.key].hasOwnProperty(account)) continue;
	...
	async trackOutput(data) {
		try {
			let embed = new Embed().setColor(this.server.colors.ratings);
	...
	async handle(data) {
		try {
			...
			let parsedData = await
				Tracker[method](sourceData, {
					"source": data.source,
					"username": data.username
				});
			data = await
				Tracker.assign(data, parsedData);
			return data;
		} catch (e) {
			throw e;
			return data;
	...
	async request(data) {
		try {
			...
			try {
				...
				if (e) throw "Please check account exists.";
			}
		} catch (e) {
			if (e) throw e;
		}
	...
	static
	async parselichess(lichessData, data) { //Parsing API Data is different for each site
		try {
			let source = data.source;
			if (!lichessData) throw "No data found."; //if invalid username (i.e. from this.track()), return;
			if (lichessData.closed) throw "Account is closed."; //if closed return
			let ratings = { //the sub-object
				"maxRating": 0
			};
			if (!lichessData.perfs) throw "No game data found.";
			...
			if (allProvisional) throw "All ratings are provisional.";
			try {
				if (lichessData.engine && lichessData.booster) throw "uses chess computer assistance and artificially increases/decreases their rating";
				if (lichessData.engine) throw "uses chess computer assistance";
				if (lichessData.booster) throw "artificially increases/decreases their rating";
			} catch (e) {
				if (e) throw `Player ` + e + ".";
			}
			delete ratings.cheating; //in case of temporary marks
			let account = {
				"ratings": ratings,
